    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute top-14 right-4 z-[130] w-72 max-h-80 bg-white/95 backdrop-blur-sm rounded-xl border border-sky-200/70 overflow-hidden flex flex-col"
        >
          {/* Header */}
          <div className="px-4 py-3 border-b border-sky-100 flex items-center justify-between">
            <span className="text-sm font-semibold text-slate-700">{t('whiteboard.history')}</span>
                ...
                {[...snapshots].reverse().map((snap, reverseIdx) => {
                  ...
                  return (
                    <div
                      ...
                      className="px-4 py-2.5 flex items-center justify-between hover:bg-sky-50/70 transition-colors group"
                    >
                      ...
                      <button
                        ...
                        disabled={isClearing}
                        className="ml-2 px-2 py-1 text-xs text-sky-700 hover:bg-sky-100 rounded-md opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                      >
